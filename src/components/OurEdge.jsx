    ...
    <section id="services" className="bg-white py-12">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="text-center mb-12" data-aos="fade-up">
          <p className="ag-semi-bold text-primary mb-6">OUR EDGE</p>
          {/* h2: 48px / 48px */}
          <h3 className="ag-h3 text-muted-foreground mb-4">Smarter Energy, by Design</h3>
          <p className="text-slate-500 max-w-[36rem] mx-auto text-sm md:text-base">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          {edges.map((edge) => (
            <div key={edge.num} className="text-center" data-aos="fade-up" data-aos-delay={parseInt(edge.num) * 100}>
              <div
                className="w-12 h-12 rounded-full bg-primary text-white font-bold text-lg flex items-center justify-center mx-auto mb-4"
                aria-hidden="true"
              >
                {edge.num}
              </div>
              {/* h5: 24px / 32px */}
              <p className="ag-semi-bold-lg text-foreground mb-4 whitespace-pre-line">{edge.title}</p>
